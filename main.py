...
from dynamic_models.dy_vgg import vgg13 as dy_vgg13
from dynamic_models.dy_vgg import vgg16 as dy_vgg16
from dynamic_models.dy_vgg import vgg19 as dy_vgg19
from dynamic_models.dy_vgg import vgg19_bn as dy_vgg19_bn
from dynamic_models.raw_vgg import vgg11 as raw_vgg11
from dynamic_models.raw_vgg import vgg11_bn
...
from conv_plot import plotting
...
def write_accuracy_log(accuracy_list):
    sf = open(args.net_name+'_accuracy.log','a+')
    sf.write(str(accuracy_list) + '%')
    sf.write('\n')
    sf.close()
...
if args.dataset == 'cifar10':
    ...
    printloader = torch.utils.data.DataLoader(testset, batch_size=1, shuffle=False, num_workers=0)
...
elif args.net_name=='dy_vgg13':
    model = dy_vgg13(num_classes=numclasses)
elif args.net_name=='dy_vgg16':
    model = dy_vgg16(num_classes=numclasses)
elif args.net_name=='dy_vgg19':
    model = dy_vgg19(num_classes=numclasses)
elif args.net_name=='dy_vgg19_bn':
    model = dy_vgg19_bn(num_classes=numclasses)
# elif args.net_name=='dy_vgg11':

#     model = vgg11_bn(num_classes=numclasses)
print(args.net_name)
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(trainloader):
        ...
        optimizer.step()
    print('Train Epoch: {}, loss{:.6f}, acc{}\n'.format(epoch, loss.item(), train_acc/len(trainloader.dataset)), end='')
...
write_accuracy_log(acc_list)
# plotting(printloader,model)
torch.save(model.state_dict(), args.net_name +'.pth')
